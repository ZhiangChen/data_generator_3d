...
def generate_centered_shapenet_object(scene, shapenet, rng, asset_id=None):
    if asset_id is None:
...
def render_depth_image(scene, renderer, asset_id):
    ...
    for view_name, cam_pos in camera_views.items():
        ...
        os.makedirs("output/shapenet_selected", exist_ok=True)
        
        # Save the depth image as PNG for this view
        depth_image_path_png = f"output/shapenet_selected/{asset_id}_{view_name}_depth_image.png"
        ...
        kb.write_png(frames_dict['rgba'][0], f"output/shapenet_selected/{asset_id}_{view_name}_rgba_image.png")
...
def find_shapenet_objects_with_limited_depth(scene, renderer, shapenet, max_depth_threshold, selected_objects):
    found_objects = []
    rng = np.random.default_rng()
    
    for asset_id in selected_objects:

        # Generate a new ShapeNet object at the center
        asset_id, obj = generate_centered_shapenet_object(scene, shapenet, rng, asset_id)
...
if __name__ == "__main__":
    # add a delay to run this script
    # import time
    # time.sleep(120 * 60)  # Sleep for 5 minutes
    ...
    found_shapenet_objects = find_shapenet_objects_with_limited_depth(
        scene, renderer, shapenet, max_depth_threshold, selected_objects
    )

    logging.info(f"Found ShapeNet objects beyong limited depth: {found_shapenet_objects}")
